...
const round = (n, places = 3) => parseFloat(n.toFixed(places));
function strDuration(n) {
    let unit = "ns";
    if (n >= 6e10) {
        let secs = n / 1e9;
...
function pushResultLines(results, output) {
...
function buildReportComment(results, benchName, curSuite, prevSuite, cc) {
...
function buildAlertComment(losses, gains, benchName, curSuite, prevSuite, threshold, cc) {
    // Do not show benchmark name if it is the default value 'Benchmark'.
